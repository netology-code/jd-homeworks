...
Перейдя по ссылке заполняем поля: First Name, Last Name, Email своими данными и в ответ (а так же на почтовый адрес) будет
выслан ключ, с этим ключем нужно делать запросы к API. И так чтобы получить ссылку на картинку нужно:
...
8. Сохраните тело ответа в файл с именем части урла;
